...
const Data = ({ data, length, userId }: AllDataSchema) => {
    const { priceRange, mileageRange, pagevalue, color, search, setpage, setparams } = useContext<any>(ParamContext);
    const dispatch = UseAppDispatch();
    const Toast = useToast();
    const token: string = JSON.parse(sessionStorage.getItem("login_cred") || "").token || "";
    const setdelete: (_id: string) => void = async (_id) => {
        try {
            await axios.delete(`${apiUrlBack}/inventory?_id=${_id}&user=${userId}`, {
    ...
    return (
        <Box w="100%" overflowY={"auto"}>
            ...
            >
                {data?.map((el: any) => {
                    return (
                        <Box key={el._id} display={"flex"} flexDir={{ base: "column", sm: "row" }} mx="auto" mt="20px" overflow="hidden" border={"2px solid #efefef"} borderRadius={"10px"} w={"100%"}>
                            <Box w="100%" display={"flex"} flexDir={"column"}>
                                <Image display={"inline-block"} mx="auto" borderBottom={"1px solid lightgray"} w="100%" src={el.image} alt={el.title} h="200px" />
                                <Flex p="10px" flexDir={"column"} w="100%">
                                    ...
                                    <Flex justifyContent={"space-between"}>
                                        ...
                                        {userId === el.user ? <ButtonDialog title="Delete" _id={el._id || ""} heading="Delete this Item" description={"Are you "} setdelete={setdelete} /> : ""}
            ...
            <Box mt="20px">
                <Flex alignItems={"center"} w={"98%"} justifyContent={"center"} m={"auto"}>
                    {
                        <Button isDisabled={pagevalue != 1 ? false : true} onClick={() => changePage(+pagevalue - 1)}>
                            Prev
                        </Button>
                    }
                    ...
                    {
                        <Button isDisabled={pagevalue != Math.ceil(+length / 8) ? false : true} onClick={() => changePage(+pagevalue + 1)}>
                            Next
                        </Button>
                    }
                </Flex>
